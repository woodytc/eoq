Ext.define('UserManagementForm', {
    ...
    constructor: function (config) {
        ...
        //me.gridStore.setpro
        UserManagementForm.superclass.constructor.apply(this, arguments);
    } // end constructor
});
//fn change role
UserManagementForm.prototype.popUpEditItem = function (dataview, record, parent,mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    
    UserManagementForm.prototype.popUpEditUserAndRoles(id, id, email, role==='User Managemant'?"UserManage":role,parent);
};

//fn reset password
UserManagementForm.prototype.popUpResetPassword = function (dataview, record, parent,mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    UserManagementForm.prototype.resetPassword(id, id, email, role,parent);
};

//fn Disable user
UserManagementForm.prototype.popUpDisableUser = function (dataview, record, parent,mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    var status = record.get('IsApproved');
    
    UserManagementForm.prototype.disableUser(id, id, email, role, status, parent);
};

//fn search
UserManagementForm.prototype.search = function (url, username,role) {
    var prefix = 'usermanagementform-';
...
//handle change role
UserManagementForm.prototype.popUpEditUserAndRoles = function (id, username, email, role,parent,mode){
      var prefix ='roleform-';
                        ...
                        */
                        ...
                        Ext.Ajax.request({
                            ...
                            success: function(response){
                                ...
                                UserManagementForm.prototype.search(window.gridData,"","All Role");
...
//fn resertpassword
UserManagementForm.prototype.resetPassword = function (id, username, email, role,parent,mode){
                        ...
                        try{
                            if(Ext.getCmp(prefix + 'password').getValue() === Ext.getCmp(prefix + 'repassword').getValue()){
                                Ext.Ajax.request({
                                    ...
                                    success: function(response){
                                        var text = response.responseText;
                                        UserManagementForm.prototype.search(window.gridData,"","");
                                        // process server response here
...
UserManagementForm.prototype.disableUser = function (id, username, email, role, status, parent,mode){
