...
MeterialForm.prototype.popUpEditItem = function (dataview, record, parent, mode) {
    ...
    MeterialForm.prototype.popUpEditUserAndRoles(id, id, email, role === 'User Managemant' ? "UserManage" : role, parent);
};

//fn reset password
MeterialForm.prototype.popUpResetPassword = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    MeterialForm.prototype.resetPassword(id, id, email, role, parent);
};

//fn Disable user
MeterialForm.prototype.popUpDisableUser = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    var status = record.get('IsApproved');

    MeterialForm.prototype.disableUser(id, id, email, role, status, parent);
};

//fn search
MeterialForm.prototype.search = function (url, username, role) {
    var prefix = 'MeterialForm-';
...
//handle change role
MeterialForm.prototype.popUpEditUserAndRoles = function (id, username, email, role, parent, mode) {
    var prefix = 'roleform-';
                ...
                */
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response) {
                        ...
                        MeterialForm.prototype.search(window.gridData, "", "All Role");
...
//fn resertpassword
MeterialForm.prototype.resetPassword = function (id, username, email, role, parent, mode) {
                ...
                try {
                    if (Ext.getCmp(prefix + 'password').getValue() === Ext.getCmp(prefix + 'repassword').getValue()) {
                        Ext.Ajax.request({
                            ...
                            success: function (response) {
                                var text = response.responseText;
                                MeterialForm.prototype.search(window.gridData, "", "");
                                // process server response here
...
MeterialForm.prototype.disableUser = function (id, username, email, role, status, parent, mode) {
