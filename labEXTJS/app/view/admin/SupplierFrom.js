Ext.define('SupplierFrom', {
    ...
    constructor: function (config) {
        ...
        Ext.apply(this, {
            ...
            items: [
                    {
                        ...
                        items: [
                                {   // column 1
                                    ...
                                    fieldDefaults: { labelAlign: 'left' },
                                    items: [
                                            { id: me.prefix + 'username', name: 'username', fieldLabel: 'UNit', emptyText: '[Username]', anchor: '-10' }
                                    ]
                                }, //end colum 1
                                { // column 2
            ...
            columns: [
            { text: 'SupplierName', dataIndex: 'UserName', width: 250, sortable: false, align: 'center' },
            { text: 'Address', dataIndex: 'Email', width: 250, sortable: false, align: 'center' },
            { text: 'Phone', dataIndex: 'Roles', width: 250, sortable: false, align: 'center' },
        ...
        //me.gridStore.setpro
        SupplierFrom.superclass.constructor.apply(this, arguments);
    } // end constructor
});
//fn change role
SupplierFrom.prototype.popUpEditItem = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');

    SupplierFrom.prototype.popUpEditUserAndRoles(id, id, email, role === 'User Managemant' ? "UserManage" : role, parent);
};

//fn reset password
SupplierFrom.prototype.popUpResetPassword = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    SupplierFrom.prototype.resetPassword(id, id, email, role, parent);
};

//fn Disable user
SupplierFrom.prototype.popUpDisableUser = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    var status = record.get('IsApproved');

    SupplierFrom.prototype.disableUser(id, id, email, role, status, parent);
};

//fn search
SupplierFrom.prototype.search = function (url, username, role) {
    var prefix = 'SupplierFrom-';
...
//handle change role
SupplierFrom.prototype.popUpEditUserAndRoles = function (id, username, email, role, parent, mode) {
    var prefix = 'roleform-';
                ...
                */
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response) {
                        ...
                        SupplierFrom.prototype.search(window.gridData, "", "All Role");
...
//fn resertpassword
UnitForm.prototype.resetPassword = function (id, username, email, role, parent, mode) {
                ...
                try {
                    if (Ext.getCmp(prefix + 'password').getValue() === Ext.getCmp(prefix + 'repassword').getValue()) {
                        Ext.Ajax.request({
                            ...
                            success: function (response) {
                                var text = response.responseText;
                                UnitForm.prototype.search(window.gridData, "", "");
                                // process server response here
...
UnitForm.prototype.disableUser = function (id, username, email, role, status, parent, mode) {
