...
CustomerForm.prototype.popUpEditItem = function (dataview, record, parent, mode) {
    ...
    CustomerForm.prototype.popUpEditUserAndRoles(id, id, email, role === 'User Managemant' ? "UserManage" : role, parent);
};

//fn reset password
CustomerForm.prototype.popUpResetPassword = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    CustomerForm.prototype.resetPassword(id, id, email, role, parent);
};

//fn Disable user
CustomerForm.prototype.popUpDisableUser = function (dataview, record, parent, mode) {
    var id = record.get('UserName');
    var email = record.get('Email');
    var role = record.get('Roles');
    var status = record.get('IsApproved');

    CustomerForm.prototype.disableUser(id, id, email, role, status, parent);
};

//fn search
CustomerForm.prototype.search = function (url, username, role) {
    var prefix = 'CustomerForm-';
...
//handle change role
CustomerForm.prototype.popUpEditUserAndRoles = function (id, username, email, role, parent, mode) {
    var prefix = 'roleform-';
                ...
                */
                ...
                Ext.Ajax.request({
                    ...
                    success: function (response) {
                        ...
                        CustomerForm.prototype.search(window.gridData, "", "All Role");
...
//fn resertpassword
CustomerForm.prototype.resetPassword = function (id, username, email, role, parent, mode) {
                ...
                try {
                    if (Ext.getCmp(prefix + 'password').getValue() === Ext.getCmp(prefix + 'repassword').getValue()) {
                        Ext.Ajax.request({
                            ...
                            success: function (response) {
                                var text = response.responseText;
                                CustomerForm.prototype.search(window.gridData, "", "");
                                // process server response here
...
CustomerForm.prototype.disableUser = function (id, username, email, role, status, parent, mode) {
